...
def receive_file(conn, hashed_sesskey):
    ...
    if rlist:
       ...
       # print('Time from first byte to receiving length:', (a - start_recv_time) * 1000, 'milliseconds')
        ...
        print(f'{extension} file received from AM1')
        print(f'Receiving time for {extension} file from AM1: ', round((receive_file_end_time - start_recv_time) * 1000,5), 'milliseconds\n')
   ...
   # print(f'-----{STEP_PHRASE}2 MSG RECEIVED-----')
    ...
    if abs(t2_2-MSG2['t2'])<=delta_T:
        ...
        if MSG2['V2']==comparison:
            ...
            #S_AD_AM_bT=mod_xp(p,S_AD_AM_a,Pr)
...
print('-----Receiving gcode file from AM1-----')

gcode_file_data,gcode_extension=receive_file(AM1,hashed_sessAMkey)

print('-----Receiving ngc file from AM1-----')
